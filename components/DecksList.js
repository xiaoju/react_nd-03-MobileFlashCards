...
export default class DecksList extends Component {
  ...
  componentDidMount() {
    getDecks().then(allDecks => this.setState({ allDecks: allDecks }))
  }
      ...
      <View style={[MyStyles.background]}>
        {Object.keys(this.state.allDecks).length === 0 && (
          <View style={[MyStyles.infoView]}>
            <Text style={[MyStyles.smallText]}>
              Press the + button to create new decks.
            </Text>
            <Text style={[MyStyles.smallText]}>
              BEWARE! The bomb button will DELETE ALL YOUR DECKS!
            </Text>
          </View>
        )}

        <FlatList
          data={Object.keys(this.state.allDecks).map(
            title => this.state.allDecks[title]
          )}
          renderItem={({ item }) => (
            <DecksListItem thisDeck={item} navigation={this.props.navigation} />
          )}
        />
        ...
        <View style={[MyStyles.iconsBar]}>
          ...
          <TouchableHighlight style={[MyStyles.iconContainer]}>
            <FontAwesome
              name="bomb"
              onPress={() =>
                deleteAllDecks().then(() => this.setState({ allDecks: {} }))
              }
              size={40}
            />
          </TouchableHighlight>
